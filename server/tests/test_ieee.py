...
from server.utils.search.ieee import search_ieee
...
def test_search_ieee_parses_response():
    ...
    with (
        patch.dict("os.environ", {"IEEE_API_KEY": "fake-key"}),
        patch("server.utils.search.ieee.http_get_json", return_value=response),
    ):
...
def test_search_ieee_handles_empty_response():
    with (
        patch.dict("os.environ", {"IEEE_API_KEY": "fake-key"}),
        patch("server.utils.search.ieee.http_get_json", return_value={}),
    ):
        assert search_ieee("test", 3, 5) == []
...
def test_search_ieee_returns_empty_when_request_fails():
    with (
        patch.dict("os.environ", {"IEEE_API_KEY": "fake-key"}),
        patch("server.utils.search.ieee.http_get_json", return_value=None),
    ):
        assert search_ieee("test", 3, 5) == []
