...
from server.utils.search.patents import (
    _dedup_patents,
    _get_epo_ops_token,
    _search_epo_ops,
    _search_lens,
...
def test_search_patentsview_parses_response():
    ...
    with patch("server.utils.search.patents.http_get_json", return_value=response):
...
def test_search_patentsview_returns_empty_when_request_fails():
    with patch("server.utils.search.patents.http_get_json", return_value=None):
...
def test_search_patentsview_skips_patents_without_number():
    response = {
        "patents": [
            ...
            {
                "patent_number": "US20000000B2",
                "patent_title": "Valid",
                "patent_date": "2019-06-15",
            },
        ]
    }
    with patch("server.utils.search.patents.http_get_json", return_value=response):
...
def test_search_epo_ops_returns_empty_without_token():
    with patch("server.utils.search.patents._get_epo_ops_token", return_value=None):
...
def test_search_patents_falls_back_to_patentsview():
    ...
    with (
        patch("server.utils.search.patents._search_epo_ops", return_value=[]),
        patch(
            "server.utils.search.patents._search_patentsview",
            return_value=[fake_patent],
        ),
    ):
...
def test_search_patents_returns_empty_gracefully():
    with (
        patch("server.utils.search.patents._search_epo_ops", return_value=[]),
        patch("server.utils.search.patents._search_patentsview", return_value=[]),
        patch("server.utils.search.patents._search_uspto", return_value=[]),
        patch("server.utils.search.patents._search_lens", return_value=[]),
    ):
...
def test_search_uspto_parses_response():
    ...
    with (
        patch.dict("os.environ", {"USPTO_API_KEY": "fake-key"}),
        patch("server.utils.search.patents.http_post_json", return_value=response),
    ):
...
def test_search_uspto_handles_empty_results():
    with (
        patch.dict("os.environ", {"USPTO_API_KEY": "fake-key"}),
        patch("server.utils.search.patents.http_post_json", return_value={}),
    ):
        assert _search_uspto("test", 3, 5) == []
...
def test_search_lens_parses_response():
    ...
    with (
        patch.dict("os.environ", {"LENS_API_TOKEN": "fake-token"}),
        patch("server.utils.search.patents.http_post_json", return_value=response),
    ):
