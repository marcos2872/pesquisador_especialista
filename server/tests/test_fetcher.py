...
from server.services.source_service import (
    _is_search_or_home_url,
    _looks_like_primary_source,
)
from server.utils.fetcher import (
...
def test_generate_query_variants_includes_connectors():
    ...
    assert any("paper" in q.lower() for q in queries)
    assert any("systematic review" in q.lower() for q in queries)
    assert len(queries) <= 10  # Limite máximo
...
def test_validate_url_relevance_validates_doi_via_crossref():
    ...
    with patch("server.utils.fetcher.urlopen") as mock_urlopen:
...
def test_download_source_texts_enriches_sources():
    ...
    with patch("server.utils.fetcher.fetch_url") as mock_fetch:
...
def test_download_source_texts_handles_fetch_failure():
    ...
    with patch("server.utils.fetcher.fetch_url", return_value=(False, "", "timeout")):
