...
def _should_retry(status_code: int | None, error: Exception | None) -> bool:
    """Determina se um erro merece retry."""
...
def _make_request(
    ...
    """Executa uma requisição HTTP e retorna (status_code, content)."""
    try:
...
def http_get_json(
    ...
    """Faz GET e retorna JSON, com retry em 429/5xx/URLError."""
    headers = headers or {}
    for attempt in range(max_retries + 1):
        ...
        if status is not None and status not in RETRYABLE_STATUS_CODES:
            # Erro definitivo (404, 403, etc.) — não retrya
            return None

        if attempt < max_retries:
            backoff = DEFAULT_BACKOFF * (attempt + 1)
            if status == 429:
                backoff = max(backoff, 2.0)  # rate limit = espera mais
            logger.warning(
...
def http_get_text(
    ...
    """Faz GET e retorna texto, com retry em 429/5xx/URLError."""
    headers = headers or {}
    for attempt in range(max_retries + 1):
        req = Request(url, headers=headers, method="GET")
        status, content = _make_request(req, timeout, attempt)
...
def http_post_json(
    ...
    """Faz POST com JSON e retorna JSON, com retry em 429/5xx/URLError."""
    headers = headers or {}
